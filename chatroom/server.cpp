...
int main(int argc,char* argv[])
{
    ...
    /*创建users数组，分配FD_LIMIT个client_data对象。可以预期：每次可能的socket连接都
     * 可以获得这样一个对象，并且socket的值可以直接用来索引（作为数组的下标）socket连接
     * 对应的client_data对象，这是将socket和客户数据关联的简单而高效的方式*/
    client_data* users = new client_data[FD_LIMIT];
    /* 尽管我们分配了足够多的client_data对象，但为了提高poll的性能，仍然有必要去限制用户的数量*/
